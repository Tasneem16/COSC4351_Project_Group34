...
def tables():
    if request.method == 'POST':
        ...
        new_table = Tables(capacity=table)
        ...
        if request.method == 'POST':
            ...
            if float(guests) < 0:
                ...
                if user_card:
                    ...
                    flash('Your Table has been Reserved!', category='success')
...
def guestpayment():
    ...
    if request.method == 'POST':
        ...
        db.session.add(new_payment)
        db.session.commit()
        flash('Your reservation will be final and no show will be charged minimum $10.', category='error')
        flash('Your Table has been Reserved!', category='success')
        return redirect(url_for('views.home'))
